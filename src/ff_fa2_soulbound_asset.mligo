...
#include "./fa2_soulbound_token.mligo"
...
let rec _bytes_to_nat(target, index, result : bytes * nat * nat) : nat =
  let bytes_nat_map : (bytes, nat) map = Map.literal [
    (0x00, 0n); (0x01, 1n);
    (0x02, 2n); (0x03, 3n);
    (0x04, 4n); (0x05, 5n);
    (0x06, 6n); (0x07, 7n);
  ...
  if Bytes.length target = 0n then (failwith "BYTES_LENGTH_ZERO" : nat)
    ...
    match Map.find_opt byte bytes_nat_map with
      | None -> (failwith "UNDEFINED_BYTES_IN_MAP" : nat)
      | Some n ->
        _bytes_to_nat(target, index+1n, result * 256n + n)
  else
    result
